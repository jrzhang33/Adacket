...
class _ConvNd(Module):
    ...
    def reset_parameters(self) -> None:
      ...
      #  init.xavier_normal_(self.weight,gain=1.0)
        ...
        self.weight = Parameter(self.weight - Parameter(self.weight.mean()))
        if self.bias is not None:
            fan_in, _ = init._calculate_fan_in_and_fan_out(self.weight) #in_channel * length
            if fan_in != 0:
                bound = 1 / math.sqrt(fan_in)
                init.uniform_(self.bias, -bound, bound)
        # init.uniform_(self.bias, a=-1, b=1)
    def extra_repr(self):
